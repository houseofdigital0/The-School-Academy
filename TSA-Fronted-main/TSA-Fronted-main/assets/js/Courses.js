...
function createCard(session,i) {
  ...
  <!-- Image Section -->
  <div class="relative">
    <img src="${`http://localhost:5000/${session.image}`}" alt="${session.name}" class="w-full  min-h-[20vh] max-h-[20vh] object-cover" loading="lazy">
...
const openCourseModel = (course) => {
  ...
  courseImage.src = `http://localhost:5000/${course.image}`; // Assuming course object has an image property
